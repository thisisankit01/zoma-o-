...
const Body = () => {
  ...
  useEffect(() => {
    //js hook with callback function and dependency array
    ...
    <>
      <div className="grid place-items-center">
        <form className="flex flex-row pt-8 w-3/4 space-x-2">
          ...
          <button
            className="py-2 bg-gray-50 px-8 rounded-md font-extralight max-sm:hidden text-slate-500"
            onClick={() => {
              const data = filterData(searchInput, allRestaurants);
              setFilteredRestaurants(data);
            }}
      ...
      <div className="grid grid-row place-items-center pb-10">
        {filteredRestaurants.length === 0 ? (
          <h1 className="grid place-content-center px-8 pt-14 font-bold text-3xl">
